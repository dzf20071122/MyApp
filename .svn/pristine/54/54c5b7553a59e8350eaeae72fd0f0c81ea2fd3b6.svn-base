...
import com.research.adapter.MettingAdapter;
...
import com.research.widget.MeetingSearchDialog;
...
public class MettingActivity extends BaseActivity implements OnChangeStateListener, OnItemClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ResearchCommon.saveReadMeetingTip(mContext, true);
		initCompent();
		...
		new Thread(){
			public void run() {
				try {
					...
					if (isExitsData) {
						mMyAlbum = ResearchCommon.getResearchInfo().meetingList(mSelectType,page);
						if ((loadType == GlobalParam.LIST_LOAD_FIRST || loadType == GlobalParam.LIST_LOAD_REFERSH)) {
						...
						if (mMyAlbum != null && mMyAlbum.childList!=null && mMyAlbum.childList.size() > 0) {
							...
							for (int i = 0; i < mMyAlbum.childList.size(); i++) {
								mMyAlbum.childList.get(i).unread = messageTable.queryUnreadCountByID(mMyAlbum.childList.get(i).id+"",GlobleType.MEETING_CHAT);
