...
public class MyPullToRefreshScrollView extends LinearLayout{
	...
	private int mCurRefreshViewHeight = 49;
	private int TOP_LAYOUTHEIGHT = 49;
	...
	private void changeState(int state) {
		...
		case STATE_RELEASE:
			mRefreshViewImage.setVisibility(View.VISIBLE);
			mRefreshProgress.setVisibility(View.GONE);
			 mRefreshViewText.setText(R.string.starttorefresh);
			changeRefreshView(false);
			break;
		case STATE_LOADING:
			mRefreshViewImage.setVisibility(View.GONE);
			mRefreshProgress.setVisibility(View.VISIBLE);
			mRefreshViewText.setText(R.string.refreshloading);
	...
	public void setRefreshViewHeight(int height) {
		...
		if (height == 1) {
			...
			mRefreshView.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, FeatureFunction.dip2px(mContext, 49)));
		} else {
	...
	public LazyScrollView getScrollView() {
		return mScrollView;
	}
	...
	protected void onFinishInflate() {
		...
		mRefreshViewImage.setImageResource(R.drawable.arrow_down);
		mScrollView = (LazyScrollView) findViewById(R.id.waterfall_scroll);
	...
	public void onRefreshComplete() {
		changeState(STATE_IDLE);
		TranslateAnimation am = new TranslateAnimation(0, 0, 100, 1);
		am.setDuration(400l);
