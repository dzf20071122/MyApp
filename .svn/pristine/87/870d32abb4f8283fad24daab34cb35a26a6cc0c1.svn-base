...
import com.research.Entity.ChatDetailEntity;
...
import com.research.adapter.ChatPersonAdapter;
			...
			if(mList.get(arg2).mType == 0){
				if(mAdapter.getIsDelete()){
					...
					if(mIsSignChat == 1){
						...
						intent.putExtra("type",2);
						if(mGroupDetail.mUserList.get(arg2).uid.equals(ResearchCommon.getUserId(mContext))){
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchJiaState statue = ResearchCommon.getResearchInfo().modifyGroupNickName(mGroupDetail.groupId, mInputGroupName);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_GROUP_NAME,statue);
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.commit_dataing));
					ResearchJiaState statue = ResearchCommon.getResearchInfo().modifyMyNickName(mGroupDetail.groupId, mInputGroupNickName);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_MY_GROUP_NAME,statue);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
				} catch (ResearchException e) {
					e.printStackTrace();
					ResearchCommon.sendMsg(mBaseHandler, BASE_MSG_TIMEOUT_ERROR,
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_CREAT_ROOM_SUCCESS:
				...
				if(room!=null){
					...
					/*	if(room.mUserList!=null && room.mUserList.size()>0){
						ResearchCommon.sendMsg(mHandler, INVITE_SUCCESS, room.mUserList);
					}*/
					mGroupDetail = room;
					if(mList!=null && mList.size()>0){
						mList.clear();
			...
			case GlobalParam.MSG_GROUP_ISGET_CHECK:
				ResearchJiaState getRecv_status = (ResearchJiaState)msg.obj;
			...
			case GlobalParam.MSG_CHECK_GROUP_NAME:
				ResearchJiaState statue = (ResearchJiaState)msg.obj;
			...
			case GlobalParam.MSG_CHECK_MY_GROUP_NAME:
				ResearchJiaState returnStatue = (ResearchJiaState)msg.obj;
					...
					if(mList!=null && mList.size()>0){
						for (int j = 0; j < mList.size(); j++) {
							if(mList.get(j).mLogin!=null){
								if(mList.get(j).mLogin.uid.equals(ResearchCommon.getUserId(mContext))){
		...
		new Thread(){
			...
			public void run(){
				if(ResearchCommon.verifyNetwork(mContext)){
					try {
						Room result = ResearchCommon.getResearchInfo().getRommInfoById(mGroupID);
	...
	private void kickPerson(final int pos){
		new Thread(){
			...
			public void run(){
				if(ResearchCommon.verifyNetwork(mContext)){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().kickParticipant(mGroupID, mList.get(pos).mLogin.uid);
						if(state != null && state.code == 0){
							ResearchCommon.sendMsg(mHandler, KICK_SUCCESS, pos);
						}else {
							Message msg=new Message();
							msg.what=GlobalParam.MSG_LOAD_ERROR;
		...
		new Thread(){
			public void run() {
				try {
					...
					Room createRoom = ResearchCommon.getResearchInfo().createRoom( mNickName, mUids);

					if(createRoom != null && createRoom.state !=null && createRoom.state.code == 0){
						ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CREAT_ROOM_SUCCESS, createRoom);
					}else {
						Message msg=new Message();
						msg.what=GlobalParam.MSG_LOAD_ERROR;
		...
		new Thread(){
			...
			public void run(){
				if(ResearchCommon.verifyNetwork(mContext)){
					...
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().inviteUsers(mGroupID, uids);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						if(state != null && state.code == 0){
							ResearchCommon.sendMsg(mHandler, INVITE_SUCCESS, userList);
						}else {
							Message msg=new Message();
							msg.what=GlobalParam.MSG_LOAD_ERROR;
							if(state != null && state.errorMsg != null && !state.errorMsg.equals("")){
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.send_request));
					ResearchJiaState stauts = null;
					if(type == 1){
						//stauts = ResearchCommon.getResearchInfo().isPublicGroup(mRoomId,recv );
					}else if(type == 2){
						if(mIsSignChat == 1){
							//setMsg
							stauts = ResearchCommon.getResearchInfo().setMsg(mGroupID);
						}else{
							stauts = ResearchCommon.getResearchInfo().isGetGroupMsg(mGroupID,recv );
						}

					}
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
					if(type == 1){
						ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_GROUP_PUBLISH_CHECK, stauts);
					}else if(type == 2){
						ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_GROUP_ISGET_CHECK, stauts);
					}
				} catch (ResearchException e) {
					e.printStackTrace();
					ResearchCommon.sendMsg(mBaseHandler, BASE_MSG_TIMEOUT_ERROR,
			...
			new Thread(){
				...
				public void run(){
					try {
						ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
								mContext.getResources().getString(R.string.send_request));
						ResearchJiaState state = ResearchCommon.getResearchInfo().deleteRoom(roomId);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						if(state != null && state.code == 0){
			...
			new Thread(){
				...
				public void run(){
					try {
						...
						ResearchJiaState state = ResearchCommon.getResearchInfo().exitRoom(roomId);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						if(state != null && state.code == 0){
							SQLiteDatabase db = DBHelper.getInstance(mContext).getWritableDatabase();
							//RoomUserTable roomUserTable = new RoomUserTable(db);
