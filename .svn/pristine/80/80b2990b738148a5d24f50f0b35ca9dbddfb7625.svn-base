	...
	private void initCompent(){
		...
		mContentTextView.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_FRIENDS_FAVORITE_DIALOG,1,0 );
				return true;
			}
		});
				...
				for (int j = 0; j < 3; j++) {
					...
					if(pos < item.listpic.size()){
						...
						view.setOnLongClickListener(new OnLongClickListener() {
							...
							@Override 
							public boolean onLongClick(View v) {
								ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_FRIENDS_FAVORITE_DIALOG,2,pos );
								return true;
							}
						});
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchJiaState status = ResearchCommon.getResearchInfo().shareReply(item.id,item.uid,content);
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.send_request));
					ResearchJiaState status = ResearchCommon.getResearchInfo().sharePraise(item.id);
					if(status!=null && status.code == 0){
						getShareDetail();
					}else{
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, 
							mContext.getResources().getString(R.string.send_request));
					ResearchJiaState status = ResearchCommon.getResearchInfo().deleteShare(item.id);
