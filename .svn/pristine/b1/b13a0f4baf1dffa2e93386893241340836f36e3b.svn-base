...
public class ResearchCommon {
		...
		if(tempList!=null && tempList.size()>0){
			for (int i = 0; i < tempList.size(); i++) {
				if(tempList.get(i).loginId!=null && !tempList.get(i).loginId.equals("")){
					if(tempList.get(i).loginId.equals(ResearchCommon.getUserId(context))){
	...
	public static void saveFriendsLoopTip(Context context, int tip){
		...
		editor.putString(LOGIN_ID, ResearchCommon.getUserId(context));
		editor.commit();
	}
	...
	public static void appOnStop(Context context){
		if (!FeatureFunction.isAppOnForeground(context) && ResearchCommon.isApplicationActive) {
			//app 进入后台
			ResearchCommon.isApplicationActive = false;
			Log.e("TAG", "app 进入后台");
	...
	public static void appOnResume(Context context){
		if (!ResearchCommon.isApplicationActive) {
			//app 从后台唤醒，进入前台
			Log.e("TAG", "app 从后台唤醒，进入前台");
			ResearchCommon.isApplicationActive = true;
