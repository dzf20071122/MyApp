...
public class ApplyMetListActivity extends BaseActivity implements OnItemClickListener{
	 ...
	 * 定义全局变量
	 */
	public static final String REMOVE_USER_ACTION = "research_remove_uesr_action";
					...
					new Thread(){
						public void run() {
							try {
								...
								if(mType == 1){
									mUser = ResearchCommon.getResearchInfo().huoyueList(currentPage,mMeetingId);
								}else{
									mUser = ResearchCommon.getResearchInfo().meetingApplyList(currentPage,mMeetingId);
								}
	...
	private void agreeApplyMeeting(final String fuid,final int pos){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().agreeApplyMeeting(mMeetingId,fuid);
						if(state!=null){
							state.positon = pos;
						}
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						ResearchCommon.sendMsg(mHandler,GlobalParam.MSG_AGREE_APPLY_METTING,state);
					} catch (ResearchException e) {
						e.printStackTrace();
	...
	private void disAgreeApplyMeeting(final String fuid,final int pos){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().disagreeApplyMeeting(mMeetingId, fuid);
						if(state!=null){
							state.positon = pos;
						}
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						ResearchCommon.sendMsg(mHandler,GlobalParam.MSG_DIS_AGREE_APPLY_METTING,state);
					} catch (ResearchException e) {
						e.printStackTrace();
						mHandler.sendEmptyMessage(GlobalParam.MSG_TICE_OUT_EXCEPTION);
					}catch (Exception e) {
	...
	private void removeMetUser(final String fuid,final int pos){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().removeMetUser(mMeetingId, fuid);
						if(state!=null){
							state.positon = pos;
						}
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_AGREE_APPLY_METTING:
				ResearchJiaState agreeStatus = (ResearchJiaState)msg.obj;
				if(agreeStatus == null){
			...
			case GlobalParam.MSG_DIS_AGREE_APPLY_METTING:
				ResearchJiaState disAgreeStatus = (ResearchJiaState)msg.obj;
