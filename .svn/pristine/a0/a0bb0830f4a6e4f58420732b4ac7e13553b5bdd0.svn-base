...
import com.research.NewFriendsActivity;
...
import com.research.Entity.Group;
...
public class ContactsFragment extends Fragment implements OnChangeStateListener {
	...
	//显示新的朋友有上角的泡泡
	public final static String ACTION_SHOW_NEW_FRIENDS = "research_action_show_new_friends_tip";
	//取消新的朋友有上角的泡泡
	public final static String ACTION_HIDE_NEW_FRIENDS = "research_action_hide_new_friends_tip";
	public final static String REFRESH_FRIEND_ACTION = "research_refresh_action";
		...
		sortListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if(login.remark!=null && !login.remark.equals("")){
					...
					}else if(login.remark.equals(mParentContext.getResources().getString(R.string.room_chat))){
						//进入我的群组页面
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent != null) {
				if (intent.getAction().equals(
					...
					if(mSourceDateList!=null && mSourceDateList.size()>0){
						mSourceDateList.get(0).newFriends =0 /*ResearchCommon.getContactTip(mParentContext)*/;
						if(mAdapter!=null){
							mAdapter.notifyDataSetChanged();
						}
	...
	private void getUserList(final int loadType) {
		new Thread() {
			...
			@Override
			public void run() {
				if (ResearchCommon.verifyNetwork(mParentContext)) {
					new Thread() {
						public void run() {
							try {
