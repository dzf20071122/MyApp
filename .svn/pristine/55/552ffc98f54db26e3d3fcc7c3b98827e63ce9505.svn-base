...
public class MyPullToRefreshGridView extends LinearLayout{
	...
	//	System.out.println("拦截");
		...
		//	System.out.println("onInterceptTouchEvent.ACTION_MOVE");
			if (mGridView.getFirstVisiblePosition() == 0
					&& (mGridView.getChildCount() == 0 || mGridView.getChildAt(0)
							.getTop() == 0)) {
	...
	protected void onFinishInflate() {
		...
		mRefreshViewImage.setImageResource(R.drawable.arrow_down);
		mGridView = (GridView) findViewById(R.id.service_gridview);
		mGridView.setSelected(false);
		mGridView.setFocusable(false);
...
//		changeState(STATE_LOADING);
	...
	public interface OnChangeStateListener {
		public void onChangeState(MyPullToRefreshGridView container, int state);
	}
