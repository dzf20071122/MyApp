...
public class MettingDetailActivity extends BaseActivity {
	 ...
	 * 定义全局变量
	 */
	public final static String ACTION_SHOW_NEW_MEETING_TIP ="research_action_show_new_meeting_tip";
	public final static String ACTION_HIDE_NEW_MEETING_TIP ="research_action_hide_new_meeting_tip";
			...
			if(intent!=null){
				...
				if(action!=null && !action.equals("")){
					if(action.equals(GlobalParam.ACTION_UPDATE_MEETING_DETAIL)){
						...
						int count = messageTable.queryUnreadCountByID(mMetId+"",GlobleType.MEETING_CHAT);
						if(count!=0){
		...
		new Thread(){
			public void run() {
				try {
					...
					MeetingItem item = ResearchCommon.getResearchInfo().mettingDetail(mMetId);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_LOAD_DATA,item);
	...
	private void applyMeeting(final String reseon){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().applyMeeting(mMetId,reseon);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						ResearchCommon.sendMsg(mHandler,GlobalParam.MSG_CHECK_APPLY_METTING,state);
					} catch (ResearchException e) {
						e.printStackTrace();
	...
	public void onClick(View v) {
		...
		case R.id.join_met_btn:
			...
			if(mMeetingItem.isjoin == 1){
				...
				user.mIsRoom = GlobleType.MEETING_CHAT;
