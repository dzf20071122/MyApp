...
public abstract class HttpHeaderFactory {
    ...
    public String getWeiboAuthHeader(String method, String url, ResearchParameters params){
    	try {
    		final long timestamp = System.currentTimeMillis() / 1000;
            final long nonce = timestamp + (new Random()).nextInt();
            // step 2: authParams�������ô���1.���ܴ�һ���� 2.������Authorizationͷ��
            ResearchParameters authParams = this.generateAuthParameters(nonce, timestamp);
            // ������ڼ���signature�ģ�����
            ResearchParameters signatureParams = this.generateSignatureParameters(authParams, params, url);
    ...
    private String generateAuthSignature(final String method, ResearchParameters signatureParams,
            final String url) {
    ...
    private ResearchParameters generateSignatureParameters(ResearchParameters authParams,
    		ResearchParameters params, String url){
    	ResearchParameters signatureParams = new ResearchParameters();
        signatureParams.addAll(authParams);
        signatureParams.addAll(params);
        this.parseUrlParameters(url, signatureParams);
        ResearchParameters lsp = generateSignatureList(signatureParams);
        return lsp;
    }

    private ResearchParameters generateAuthParameters(long nonce, long timestamp){
    	ResearchParameters authParams = new ResearchParameters();
    ...
    public abstract ResearchParameters generateSignatureList(ResearchParameters bundle);

    // add additional parameters to des key-value pairs,support to expanding
    // params
    public abstract void addAdditionalParams(ResearchParameters des, ResearchParameters src);

    // ����url�в����,�洢��signatureBaseParams
    public void parseUrlParameters(String url, ResearchParameters signatureBaseParams){
    ...
    public static String encodeParameters(ResearchParameters postParams, String splitter, boolean quot) {
