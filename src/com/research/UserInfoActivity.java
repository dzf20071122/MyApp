...
import com.research.Entity.Picture;
...
public class UserInfoActivity extends BaseActivity  implements OnClickListener{
	...
	mPhoneView,mXuanyanView,mYaoqiuView,xy_text,yaoqiu_text;
	private LinearLayout mAlbumLayout;

	private RelativeLayout mSingLayout,mPhotoLayout,mAddrLayout,mXuyanLayout,mYaoqiuLayout;

	private ImageLoader mImageLoader = new ImageLoader();
	private Login mLogin;
	private int mType = 0;
	private int mIsBlackJump =0;
	private String mSearchUid,mFromActivity;
	...
	private void initComponent(){
		if(mLogin!=null && mLogin.uid.equals(ResearchCommon.getUserId(mContext))){
		...
		mXuyanLayout = (RelativeLayout)findViewById(R.id.xuyan_layout);
		mYaoqiuLayout = (RelativeLayout)findViewById(R.id.yaoqiu_layout);
		
		xy_text = (TextView) findViewById(R.id.xy_text);
		yaoqiu_text = (TextView) findViewById(R.id.yaoqiu_text);
		
		mXuanyanView = (TextView) findViewById(R.id.xuanyan_content);
		mYaoqiuView = (TextView) findViewById(R.id.yaoqiu_content);
		
		if(mFromActivity!=null){
			mXuyanLayout.setVisibility(View.VISIBLE);
			mYaoqiuLayout.setVisibility(View.VISIBLE);
			if (mFromActivity.equals("BlindDateActivity")){
				xy_text.setText("相亲宣言");
				yaoqiu_text.setText("相亲要求");
				mXuanyanView.setText(mLogin.blindDate);
				mYaoqiuView.setText(mLogin.blindDateRequre);
			}else if (mFromActivity.equals("MakeFriendActivity")){
				xy_text.setText("交友宣言");
				yaoqiu_text.setText("交友要求");
				mXuanyanView.setText(mLogin.makeFriendXuanyan);
				mYaoqiuView.setText(mLogin.makeFriendRequre);
			}
		}
	...
	private void showProfile(){
		...
		if(mLogin.uid.equals(ResearchCommon.getUserId(mContext))){
			mRightBtn.setVisibility(View.GONE);
...
//				mSendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.send_msg));
				...
				//dzf_add
				mSendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.give_presents));
				mAddFriendBtn.setVisibility(View.VISIBLE);
				mAddFriendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.send_msg));
			}else {
				if(mLogin.userType == 1){
					if(mPopList!=null && mPopList.size()>0){
						mPopList.clear();
...
//					mSendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.send_msg));
					...
					mSingLayout.setVisibility(View.GONE);
//					mSendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.add_to_contact));
					mSendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.give_presents));
					mAddFriendBtn.setVisibility(View.VISIBLE);
					mAddFriendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.add_to_contact));
				}

				//mAddFriendBtn.setText(BMapApiApp.getInstance().getResources().getString(R.string.add_to_friend));
			}
		...
		if(mLogin != null){
			if(mLogin.headsmall != null && !mLogin.headsmall.equals("")){
				Object obj=null;
				obj=mImageLoader.getBitmap(mContext, mHeaderView, null, mLogin.headsmall, 0, false, true);

				//obj=mImageLoader.getBitmap(mContext, mHeaderView, null,mLogin.headsmall, 0,true,false);
				//Toast.makeText(mContext,mLogin.headsmall+" "+obj,Toast.LENGTH_SHORT).show();
			}
			//showAddr();
			...
			if (size > 0) {
				for (int i = 0; i < size; i++) {
					...
					if (url != null && !url.equals("")) {
						mImageLoader.getBitmap(mContext, imageView, null, url, 0, true, false);
...
//				}
		...
		case R.id.add_friends://添加好友
			if(mLogin!=null){
				//dzf_modify 添加好友，发送消息
				if (mAddFriendBtn.getText().equals(BMapApiApp.getInstance().getResources().getString(R.string.send_msg))){
					//发送消息
					if(mLogin.isfriend == 1 || mLogin.userType == 1){
				...
				}else if (mAddFriendBtn.getText().equals(BMapApiApp.getInstance().getResources().getString(R.string.add_to_contact))){
					//添加好友
					if(mLogin.isfriend == 0){
//						ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,R.string.add_more_loading);
						createDialog(mContext,
								mContext.getResources().getString(R.string.request_doing)
								,2,mContext.getResources().getString(R.string.send));
					}
	...
	private void addBlock(){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().addBlock(mLogin.uid);
	...
	private void deleteFriend(){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().cancleFriends(mLogin.uid);
						if(state != null && state.code == 0){
							...
							List<NewFriendItem> list = ResearchCommon.getNewFriendItemResult(mContext);
							if(list!=null && list.size()>0){
								for (int i = 0; i < list.size(); i++) {
									if(list.get(i).uid.equals(mLogin.uid)){
										list.get(i).type = 0;
										ResearchCommon.saveNewFriendsResult(mContext, list,0);
										break;
									}
								}
							}
	...
	private void addFriend(final String reseon){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().applyFriends(ResearchCommon.getUserId(mContext), mLogin.uid,reseon);
						if(state != null && state.code == 0){
							String hintMsg = "";
							int checkStatus = 4;
							...
							ResearchCommon.sendMsg(mHandler,GlobalParam.HIDE_PROGRESS_DIALOG,hintMsg,4);
			...
			new Thread(){
				...
				public void run(){
					try {
						...
						if(searchType == 1){
							loginResult = ResearchCommon.getResearchInfo().getUserInfo(uid);
						}else if(searchType == 2){
							loginResult = ResearchCommon.getResearchInfo().getUserByName(mSearchName);
						}
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchJiaState state = ResearchCommon.getResearchInfo().addfocus(mLogin.uid);
		...
		new Thread(){
			public void run() {
				try {
					...
					LoginResult state = ResearchCommon.getResearchInfo().setStar(mLogin.uid);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_STAR,state);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
				} catch (ResearchException e) {
					e.printStackTrace();
					ResearchCommon.sendMsg(mBaseHandler, BASE_MSG_TIMEOUT_ERROR, 
	...
	private void checkFriendsNotify(Login login,int type){
		...
		mUserList.add(new NewFriendItem(login.phone,login.uid,login.nickname,login.headsmall,
				type,"",ResearchCommon.getUserId(mContext),1));
