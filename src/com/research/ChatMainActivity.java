...
public class ChatMainActivity  extends BaseActivity implements OnItemLongClickListener, OnClickListener, OnTouchListener, OnPageChangeListener{
	...
	private RelativeLayout mChatBottmLayout,mKeyboradLayout;

	private ListView mListView;
	private Button mMsgSendBtn, mCameraBtn, mGalleryBtn, mEmotionBtn, mLocationBtn,mCardBtn,mFavoritebtn,mCallBtn,mLiveBtn;
	...
	private final static int SETEDITTEXT=0x000;
	private int BottmLayoutWidth,BottmLayoutHeight,EditTextWidth,EditTextHeight;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initComponent();
		Log.i("Runt", String.format("%s %s %s %s %s", BottmLayoutWidth,BottmLayoutHeight,EditTextWidth,EditTextHeight,mChatBottmLayout.getWidth()));
		...
		//PublicWay.activityList
		
		
		/*if(isback){//判断是否从通知栏打开此界面
			if(messageInfos.size()>0){
				try {
					content=messageInfos.get(messageInfos.size()-1).content;
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				if(content.equals(mContext.getString(R.string.sessioning_reqite))){//判断请求是否开启语音
					startVoiceCall(true);
				}else if(content.equals(mContext.getString(R.string.sessioning_reqite_v))){//判断请求是否开启语音
					startVideoCall(true);
				}
			}
		}*/
	...
	protected void onResume() {
		super.onResume();
		Log.i("Runt", String.format("tempSelectBitmap:"+Bimp.tempSelectBitmap.size()+" "+PublicWay.activityList.size()));
		if(Bimp.tempSelectBitmap.size()>0){
			int i=0;
			for(ImageItem img:Bimp.tempSelectBitmap){
				i++;
				sendFile(MessageType.PICTURE, img.imagePath);
				Log.i("Runt", String.format("ImageItem%s: imageId:%s thumbnailPath:%s imagePath:%s",i,img.imageId,img.thumbnailPath,img.imagePath));
			}
...
/*
	...
	protected void onResume() {
		...
		super.onResume();
		if(CallActivity.exception){//如果通话为异常
			if(messageInfos!=null){
				if(messageInfos.size()>0){
					MessageInfo mi=null;
					try {
						mi=messageInfos.get(messageInfos.size()-1);
						content=messageInfos.get(messageInfos.size()-1).content;
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
...
*/
	...
	private void initComponent(){
		...
		mKeyboradLayout = (RelativeLayout)findViewById(R.id.noraml_keyborad_layout);
		BottmLayoutWidth=mChatBottmLayout.getWidth();
		BottmLayoutHeight=mChatBottmLayout.getHeight();
        ...
        mCallBtn = (Button) findViewById(R.id.chat_box_expra_btn_call);
        mCallBtn.setOnClickListener(this);
        mLiveBtn = (Button) findViewById(R.id.chat_box_expra_btn_live);
        mLiveBtn.setOnClickListener(this);
        //(findViewById(R.id.chat_box_expra_rela_live)).setVisibility(View.INVISIBLE);
        //(findViewById(R.id.chat_box_expra_rela_call)).setVisibility(View.INVISIBLE);
        
		mContentEdit = (EditText) findViewById(R.id.chat_box_edit_keyword);
		EditTextHeight=mContentEdit.getHeight();
		EditTextWidth=mContentEdit.getWidth();
		mContentEdit.setOnClickListener(this);
		
		mContentEdit.addTextChangedListener(new EditChangedListener());
		...
		if(mType == GlobleType.SINGLE_CHAT){//单聊模式
	        ...
	        (findViewById(R.id.chat_box_expra_rela_live)).setVisibility(View.VISIBLE);
	        (findViewById(R.id.chat_box_expra_rela_call)).setVisibility(View.VISIBLE);
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case MSG_RESIZE:
				...
				break;
			case SETEDITTEXT:
				float fontsize=mContentEdit.getTextSize();
				int cha=(int)((mContentEdit.getLineCount()-1)*fontsize);
				//设置底部控件的高;
				mChatBottmLayout.setLayoutParams(new LinearLayout.LayoutParams(BottmLayoutWidth, BottmLayoutHeight+cha));
				mContentEdit.setHeight(EditTextHeight+cha);
	    	   
				android.view.ViewGroup.LayoutParams lp = mContentEdit.getLayoutParams();
				lp.height=EditTextHeight+cha;
				//设置输入框的高
				mContentEdit.setLayoutParams(lp);
				Log.i(TAG, " 正在更改大小");
				break;
			case SEND_SUCCESS:
	...
	private void sendText() {
		...
		if (str != null
			...
			msg.readState = 1;
			Log.i("Runt", "发送消息"+fCustomerVo.uid);
	...
	private void sendFile(int type, String filePath){
		Log.i("Runt","sendFile filePath:"+filePath);
		MessageInfo msg = new MessageInfo();
		msg.fromid = ResearchCommon.getUserId(mContext);
		msg.tag = UUID.randomUUID().toString();
		msg.fromname = mLogin.nickname;
	...
	class MyAdapter extends BaseAdapter{
		...
		public MyAdapter(){
			...
			if(mType == GlobleType.GROUP_CHAT){//群聊模式
				Log.i("Runt", "群聊模式");
				SQLiteDatabase db = DBHelper.getInstance(mContext).getWritableDatabase();
		...
		private void bindView(ViewHolder viewHolder, final MessageInfo messageInfo,
			...
			if(messageInfo.typefile == MessageType.VOICE){
				...
				param.addRule(RelativeLayout.BELOW, viewHolder.nickName.getId());
				if(type == 0){
				...
				param.topMargin= FeatureFunction.dip2px(mContext, 5);//解决语音消息遮挡昵称
				if(mType == GlobleType.GROUP_CHAT){
					Log.i("Runt", "bindView显示昵称");
				}else{
					Log.i("Runt", "bindView不显示昵称");
	...
	private BroadcastReceiver chatReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(SnsService.ACTION_CONNECT_CHANGE.equals(action)){
			...
			}else if(NotifyChatMessage.ACTION_NOTIFY_CHAT_MESSAGE.equals(action)){//在会话框中接收到新消息事件
				...
				/*if(msg.content.equals(mContext.getString(R.string.sessioning_reqite))){//判断请求是否开启语音
					startVoiceCall(true);
				}else if(msg.content.equals(mContext.getString(R.string.sessioning_reqite_v))){//判断请求是否开启语音
					startVideoCall(true);
				}*/
	...
	public void onClick(View v) {
        ...
        case R.id.chat_box_expra_btn_call:
        	//开启语音聊天
			Message msg=new Message();
			msg.what=BASE_SHOW_PROGRESS_DIALOG;
			msg.obj=BMapApiApp.getInstance().getResources().getString(R.string.get_dataing);
			mBaseHandler.sendMessage(msg);
        	startVoiceCall(false);
        	BaseActivity.isCall=true;
            break;
        case R.id.chat_box_expra_btn_live:
			Message msg1=new Message();
			msg1.what=BASE_SHOW_PROGRESS_DIALOG;
			msg1.obj=BMapApiApp.getInstance().getResources().getString(R.string.get_dataing);
			mBaseHandler.sendMessage(msg1);
        	//开启视频聊天
	 ...
	 * 开启语音会话
	 */
	public void startVoiceCall(final boolean isComingCall){
		new Thread(){
			@Override
			public void run(){
					Looper.prepare();//用来在线程中支持显示Toast
				Object obj=HttpRequestServer.toGetUserInfo(fCustomerVo.uid);
				Log.i("Runt",obj.toString());
				try {
					Map<String,String> map=(Map<String,String>)obj;
					fCustomerVo.phone=map.get("phone");
			    	...
			    	callIntent.putExtra("mydata", mLogin);
			    	callIntent.putExtra("todata", fCustomerVo);
			    	callIntent.putExtra("username", fCustomerVo.phone);
			    	callIntent.putExtra("isComingCall", isComingCall);
			    	Log.i("Runt",String.format("开启语音会话 %s %s", mLogin.phone,fCustomerVo.phone));
			    	startActivity(callIntent);
			        //Toast.makeText(mContext, fCustomerVo.phone, Toast.LENGTH_SHORT) .show(); 
				} catch (Exception e) {
	 ...
	 * 开启视频会话
	 */
	public void startVideoCall(final boolean isComingCall){
		...
		new Thread(){
			...
			public void run(){
				...
				try {
			    	...
			    	callIntent.putExtra("username", fCustomerVo.phone);
			    	callIntent.putExtra("isComingCall", isComingCall);
			    	Log.i("Runt",String.format("开启视频会话 %s %s", mLogin.phone,fCustomerVo.phone));
			    	startActivity(callIntent);
			        //Toast.makeText(mContext, fCustomerVo.phone, Toast.LENGTH_SHORT) .show(); 
				} catch (Exception e) {
			        Toast.makeText(mContext, "获取用户信息失败", Toast.LENGTH_SHORT) .show(); 
	...
	public void checkToday(){
		...
		new Thread(){
			...
			public void run(){
		        Looper.prepare();//用来在线程中支持显示Toast
		        int index=HttpRequestServer.chatList.toString().indexOf(fCustomerVo.uid);//判断你好友id是否存在已经聊天的list集合
		        if(index>-1){
		        	return;
		        }
				Object flag=HttpRequestServer.toGetTodayChat(mLogin.uid,fCustomerVo.uid);
				...
				if (success!=null) {
					if (success.get("success").equals("true")) {
						...
						int integral=Integer.parseInt(success.get("integral").toString());
						if(integral>0){
							Toast.makeText(mContext,
									"每日聊天赠送" + integral + "积分",
									Toast.LENGTH_SHORT).show();
							HttpRequestServer.chatList.add(fCustomerVo.uid);//添加到已经聊过天的list集合中
							try {
								UserFolder uf = new UserFolder(mLogin.uid);// 打开用户的文件夹
								File file=uf.EditFile(uf.folders[1]+"/"+handleDate.getDateToInt(handleDate.getDateToInt()));//添加今天的日期文件
								FileWriter fw=new FileWriter(file);
								BufferedWriter  bw=new BufferedWriter(fw);
								bw.write(fCustomerVo.uid+"\t\n");
								bw.close();
								fw.close();
							} catch (IOException e) {
								// TODO Auto-generated catch block
	...
	class EditChangedListener implements TextWatcher {  
	       ...
	       public void onTextChanged(CharSequence s, int start, int before, int count) {
	    	   ...
	    	   //主线程去设置控件的大小
	    	   mHandler.sendEmptyMessage(SETEDITTEXT);
	    	   String tip=String.format("bar高：%s 文本长度：%s 换行符数量：%s 文本框高：%s",mChatBottmLayout.getHeight(),s.toString().getBytes().length,s.toString().split("\n").length,mContentEdit.getLineCount());
	    	   Log.i(TAG, tip);
