...
import com.research.treeview.TreeView;
...
public class MyPullToRefreshTreeView extends LinearLayout{
	...
	public MyPullToRefreshTreeView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	...
	private void changeState(int state) {
		...
		case STATE_PULL:
			...
			mRefreshProgress.setVisibility(View.GONE);
			 mRefreshViewText.setText(BMapApiApp.getInstance().getResources().getString(R.string.continuetorefresh));
			changeRefreshView(true);
			break;
		case STATE_RELEASE:
			mRefreshViewImage.setVisibility(View.VISIBLE);
			mRefreshProgress.setVisibility(View.GONE);
			 mRefreshViewText.setText(BMapApiApp.getInstance().getResources().getString(R.string.starttorefresh));
			changeRefreshView(false);
			break;
		case STATE_LOADING:
			mRefreshViewImage.setVisibility(View.GONE);
			mRefreshProgress.setVisibility(View.VISIBLE);
			mRefreshViewText.setText(BMapApiApp.getInstance().getResources().getString(R.string.refreshloading));
	...
	//	System.out.println("拦截");
		...
		//	System.out.println("onInterceptTouchEvent.ACTION_MOVE");
			if (mTreeView.getFirstVisiblePosition() == 0
					&& (mTreeView.getChildCount() == 0 || mTreeView.getChildAt(0)
							.getTop() == 0)) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_CANCEL:
			...
			if (mState == STATE_RELEASE) {
				...
				am.setDuration(300);
				startAnimation(am);
				mTreeView.setFocusable(false);
				mTreeView.setSelected(false);
				mRefreshViewImage.clearAnimation();
				refresh();
	...
	protected void onFinishInflate() {
		...
		mRefreshViewImage.setImageResource(R.drawable.arrow_down);
		mTreeView = (TreeView) findViewById(R.id.service_treeview);
		mTreeView.setSelected(false);
		mTreeView.setFocusable(false);
	...
	public void onRefreshComplete() {
		mTreeView.setFocusable(true);
		mTreeView.setSelected(true);
