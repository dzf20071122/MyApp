...
import com.research.adapter.FriendsLoopAdapter;
...
public class FriensLoopActivity extends BaseActivity implements OnTouchListener, OnChangeStateListener, OnPageChangeListener{
	 ...
	 * 定义全局变量
	 */
	public final static String MSG_REFRESH_MOVIINF = "research_refresh_friends_loop_action";
	...
	private void initCompent(){
		...
		mImageLoader.getBitmap(mContext, mProfileHeaderIcon,null, ResearchCommon.getLoginResult(mContext).headsmall,0,false,false);
		...
		String coverUrl = ResearchCommon.getLoginResult(mContext).cover;
		if(coverUrl!=null && !coverUrl.equals("")){
			mImageLoader.getBitmap(mContext, mHeaderBg, null, ResearchCommon.getLoginResult(mContext).cover, 0, false, false);
		...
		mLoginUserName.setText(ResearchCommon.getLoginResult(mContext).nickname);
			...
			public void run() {
				try {
					...
					if (isExitsData) {
						...
						mMyAlbum = ResearchCommon.getResearchInfo().shareList(page);
					...
					/*	if ((loadType == GlobalParam.LIST_LOAD_FIRST || loadType == GlobalParam.LIST_LOAD_REFERSH)
						...
						ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CLEAR_LISTENER_DATA,tempList,loadType);
					}
	...
	public void onClick(View v) {
		...
		case R.id.header_icon:
			...
			myAlbumIntent.putExtra("toUserID",ResearchCommon.getUserId(mContext));
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, mContext.getResources().getString(R.string.send_request));
					ResearchJiaState status = ResearchCommon.getResearchInfo().uploadUserBg(
							ResearchCommon.getUserId(mContext), mListpic);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_UPLOAD_STATUS,status);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
				} catch (ResearchException e) {
					e.printStackTrace();
					ResearchCommon.sendMsg(mBaseHandler, GlobalParam.MSG_TICE_OUT_EXCEPTION,
							mContext.getResources().getString(e.getStatusCode()));
				}catch (Exception e) {
					e.printStackTrace();
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
	 ...
	 * 下拉刷新
	 * (non-Javadoc)
	 * @see com.research.widget.MyPullToRefreshListView.OnChangeStateListener#onChangeState(com.research.widget.MyPullToRefreshListView, int)
	 */
	@Override
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_SHOW_BOTTOM_COMMENT_MENU:
				...
				mToUserId = ResearchCommon.getUserId(BMapApiApp.getInstance());//commentMvoing.uid;
				mPosition = commentIndex;
				break;
			case GlobalParam.MSG_SHOW_BOTTOM_COMMENT_REPLYTO:
				for (int i = 0; i < mDataList.size(); i++) {
					if(mDataList.get(i).showView == 1){
						mDataList.get(i).showView = 0;
					}
				...
				int commentIndex1 = msg.arg1;
				
				FriendsLoopItem commentMvoing1 = mDataList.get(commentIndex1);
				mBottomLayout.setVisibility(View.VISIBLE);

				TranslateAnimation animation1 = new TranslateAnimation(mMetric.widthPixels, 0, 0, 0);
				animation1.setDuration(500);
				animation1.setAnimationListener(mAnimationListener);
				mBottomLayout.startAnimation(animation1);
				
				CommentUser toUser = (CommentUser)msg.obj;
				
				mShareId = commentMvoing1.id;
				mToUserId = toUser.uid;
				mPosition = commentIndex1;
				break;
			...
			case GlobalParam.MSG_COMMENT_STATUS:
				ResearchJiaState commentResult = (ResearchJiaState)msg.obj;
				...
				if(mDataList!=null && mDataList.size()>=mPosition){
					...
					mDataList.get(mPosition).replylist.add(new CommentUser(mShareId, ResearchCommon.getUserId(mContext),
							ResearchCommon.getLoginResult(mContext).nickname,
			...
			case GlobalParam.MSG_ZAN_STATUS:
				ResearchJiaState zanResult = (ResearchJiaState)msg.obj;
				if(zanResult == null){
			...
			case GlobalParam.MSG_CHECK_DEL_SHARE_STATUS:
				...
				List<NotifiyVo> tempMsgList = ResearchCommon.getMovingResult(mContext);
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchJiaState status = ResearchCommon.getResearchInfo().shareReply(mShareId,mToUserId,content);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.send_request));
					ResearchJiaState status = ResearchCommon.getResearchInfo().sharePraise(mShareId);
					if(status !=null && status.code == 0){
						FriendsLoopItem shareDetail = ResearchCommon.getResearchInfo().shareDetail(mShareId);
						if(shareDetail !=null){
							status.friendsLoopitem = shareDetail;
						}
					}
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_ZAN_STATUS,status);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
				} catch (ResearchException e) {
					e.printStackTrace();
					ResearchCommon.sendMsg(mBaseHandler,BASE_MSG_TIMEOUT_ERROR,
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.send_request));
					ResearchJiaState status = ResearchCommon.getResearchInfo().favoreiteMoving(mToUserId, null, favoriteContent);
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, 
							mContext.getResources().getString(R.string.send_request));
					ResearchJiaState status = ResearchCommon.getResearchInfo().deleteShare(shareId);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_DEL_SHARE_STATUS, status);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
