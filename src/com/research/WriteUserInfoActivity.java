...
public class WriteUserInfoActivity extends BaseActivity implements OnClickListener{
	...
	private void initComponent(){
		...
		if(mType == GlobleType.MODIFY_GROUP_INFO ){
			...
			setTitleContent(R.drawable.back_btn, R.drawable.ok_btn, R.string.user_true_name);
		}
		else{
		...
		if(mType == GlobleType.COMPLETE_SEX){
			...
			mNameText.setSingleLine(false);
			createLinearLayout();
			titileTextView.setText(mContext.getResources().getString(R.string.make_friend_xuanyan));//
			mNameText.setHint(mContext.getResources().getString(R.string.make_friend_xuanyan));
		}else if(mType == GlobleType.COMPLETE_MKFRIEND_REQUIRE){
			mNameText.setSingleLine(false);
			createLinearLayout();
			titileTextView.setText(mContext.getResources().getString(R.string.make_friend_require));//
			mNameText.setHint(mContext.getResources().getString(R.string.make_friend_require));
		}else if(mType == GlobleType.COMPLETE_XIANGQIN_XUANYAN){
			mNameText.setSingleLine(false);
			createLinearLayout();
			titileTextView.setText(mContext.getResources().getString(R.string.xiangqin_xuanyan));//
			mNameText.setHint(mContext.getResources().getString(R.string.xiangqin_xuanyan));
		}else if(mType == GlobleType.COMPLETE_XIANGQIN_REQUIRE){
			mNameText.setSingleLine(false);
			createLinearLayout();
			titileTextView.setText(mContext.getResources().getString(R.string.xiangqin_require));//
			mNameText.setHint(mContext.getResources().getString(R.string.xiangqin_require));
		}
			...
			if(mType == GlobleType.COMPLETE_NICKNAME){
				...
				if(content!=null && !content.equals("")){
					if( content.length()>=2 && content.length()<=8){
						...
						Toast.makeText(mContext, mContext.getResources().
								getString(R.string.group_name_length_hint),Toast.LENGTH_LONG).show();
						return;
					}
				}
			}else if(mType == GlobleType.MODIFY_GROUP_NICKNAME){
				...
				if(content!=null ){
					if(content.length()<=40){
						intent.putExtra("mk_xuanyan", content);
					}else{
						Toast.makeText(mContext, mContext.getResources().
								getString(R.string.write_content_tit),Toast.LENGTH_LONG).show();
						return;
					}
				...
				if(content!=null){
					if(content.length()<=40){
						intent.putExtra("mk_require", content);
					}else{
						Toast.makeText(mContext, mContext.getResources().
								getString(R.string.write_content_tit),Toast.LENGTH_LONG).show();
						return;
				...
				if(content!=null ){
					if(content.length()<=10){
						intent.putExtra("true_name", content);
					}else{
						Toast.makeText(mContext,"超过字数限制",Toast.LENGTH_LONG).show();
						return;
					}
				...
				if(content!=null ){
					if(content.length()<=40){
						intent.putExtra("xq_xuanyan", content);
					}else{
						Toast.makeText(mContext,mContext.getResources().
								getString(R.string.write_content_tit),Toast.LENGTH_LONG).show();
						return;
				...
				if(content!=null ){
					if(content.length()<=40){
						intent.putExtra("xq_require", content);
					}else{
						Toast.makeText(mContext,mContext.getResources().
								getString(R.string.write_content_tit),Toast.LENGTH_LONG).show();
						return;
