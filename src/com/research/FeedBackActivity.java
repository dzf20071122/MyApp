...
public class FeedBackActivity extends BaseActivity implements OnClickListener{
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_LOAD_ERROR:
				...
				if(error_Detail != null && !error_Detail.equals("")){
					...
					Toast.makeText(mContext,BMapApiApp.getInstance().getResources().getString(R.string.feedback_fail),Toast.LENGTH_LONG).show();
				}
				break;
	...
	public void onClick(View v) {
		...
		case R.id.right_btn:
			...
			if(content.equals("")){
				Toast.makeText(mContext, BMapApiApp.getInstance().getResources().getString(R.string.input_feedback_content), Toast.LENGTH_SHORT).show();
				return;
			}
	...
	private void feedback(final String content){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().feedback(content);
						if(state != null && state.code == 0){
							...
							if(state != null && state.errorMsg != null && !state.errorMsg.equals("")){
								...
								msg.obj = BMapApiApp.getInstance().getResources().getString(R.string.feedback_fail);
							}
