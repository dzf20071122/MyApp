		...
		case R.id.right_btn:
			...
			if(remark!=null && !remark.equals("")){
				if( remark.length()>=2 && remark.length()<=8){
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, mContext.getResources().getString(R.string.commit_dataing));
					remarkFriend(remark);
				}else{
	...
	private void remarkFriend(final String remark){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = ResearchCommon.getResearchInfo().remarkFriend(mFuid, remark);
						if(state != null && state.code == 0){
							Intent intent = new Intent();
							intent.putExtra("markName", remark);
							setResult(RESULT_OK, intent);
							ResearchCommon.sendMsg(mBaseHandler, BASE_HIDE_PROGRESS_DIALOG, 
