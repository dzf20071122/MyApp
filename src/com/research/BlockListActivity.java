...
public class BlockListActivity extends BaseActivity implements OnItemClickListener, OnClickListener{
	...
	public static final String CANCEL_ACTION = "research_cancel_action";
	public static final String CANCEL_FOLLOW_ACTION = "research_cancel_follow_action";
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case CANCEL_SUCCESS:
				hideProgressDialog();
				Toast.makeText(mContext, BMapApiApp.getInstance().getResources().getString(R.string.cancel_block_success),Toast.LENGTH_LONG).show();
				getBlockList(GlobalParam.LIST_LOAD_FIRST,mType);
			...
			case CANCEL_FAILED:
				hideProgressDialog();
				Toast.makeText(mContext, BMapApiApp.getInstance().getResources().getString(R.string.cancel_block_failed),Toast.LENGTH_LONG).show();
				break;
						...
						public void run() {
							try {
								if(type == GlobleType.BLOCKLISTACTIVITY_BLOCK_TYPE){
									mUser = ResearchCommon.getResearchInfo().getBlockList(/*currentPage*/);
								}
	...
	private void cancelBlock(final String fuid,final int isFollow){
		if(ResearchCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						ResearchJiaState state = null;
						if(mType == 1){
							state = ResearchCommon.getResearchInfo().addfocus(fuid/*,isFollow*/);
						}else{
							state=ResearchCommon.getResearchInfo().cancelBlock(fuid);
						}
